...
function switchTrack(track) {
	...
	document
		.getElementById("trackDsa")
		.classList.toggle("active", track === "dsa");
	document
		.getElementById("trackAi")
		.classList.toggle("active", track === "ai");
	topics = [];
	loadTopics().then(() => {
		buildSidebar();
		updateHeroForTrack();
...
async function verifyToken() {
	...
	if (longest > 0)
		s += ` <span style="color:var(--blue);font-size:11px">(best: ${longest})</span>`;
...
function showLoading() {
	...
	document.getElementById("topBarFill").style.width = "60%";
}
function hideLoading() {
	const f = document.getElementById("topBarFill");
	f.style.width = "100%";
	setTimeout(() => {
...
async function toggleSolved(questionId, fromModal = false) {
	...
	try {
		...
		showToast(
			wasSolved ? "↩ Unmarked" : "✅ Marked solved!",
			wasSolved ? "" : "success",
		);
...
async function toggleRevision(questionId, fromModal = false) {
	...
	if (previous) {
		...
		if (nextRevisionMarked || previous.status === "solved") {
			progressMap[questionId] = {
				...
				revisionMarkedAt: nextRevisionMarked
	...
	updateAllStats();
	try {
		await apiFetch("/progress/revision", {
			method: "POST",
			body: JSON.stringify({ questionId }),
		});
		refreshModalIfOpen(questionId);
...
function updateRowUI(questionId) {
	...
	if (row) {
		...
		if (isSolved) {
			...
			setTimeout(() => row.classList.remove("just-solved"), 400);
		}
...
function updateTopicBar(topicId) {
	...
	const navProg = document.querySelector(
		`.nav-item[data-id="${topicId}"] .nav-prog`,
	);
	if (navProg) navProg.textContent = `${done}/${t.questions.length}`;
	const navItem = document.querySelector(`.nav-item[data-id="${topicId}"]`);
	if (navItem)
		navItem.classList.toggle("all-done", done === t.questions.length);
}
	...
	topics.forEach((t) =>
		t.questions.forEach((q) => {
			...
			if (isQuestionSolved(q._id)) {
				...
				if (q.difficulty === "Easy") eD++;
				else if (q.difficulty === "Medium") mD++;
				else hD++;
			}
		}),
	);
...
function buildSidebar() {
	...
	if (currentTrack === "ai") {
		...
		topics.forEach((t) => {
			...
			if (!phaseGroups[key])
				phaseGroups[key] = { label: t.phase || "Phase", items: [] };
		...
		Object.entries(phaseGroups)
			...
			.forEach(([idx, group]) => {
				...
				ph.style.cssText =
					"padding: 10px 16px 4px; font-size: .65rem; text-transform: uppercase; letter-spacing: .08em; color: var(--text-dim); font-weight: 700;";
				ph.textContent =
					(group.label || "").replace(/^Phase \d+:\s*/, "") ||
					`Phase ${idx}`;
				nav.appendChild(ph);
...
function addNavItem(nav, t) {
	...
	el.addEventListener("click", (e) => {
		...
		setTimeout(() => {
			const el =
				document.getElementById("topic-" + t._id) ||
				document.getElementById("aicard-group-" + t._id);
	...
	topics.forEach((t, ti) => {
      ...
      <div class="topic-body${isOpen ? " open" : ""}" id="tbody-${t._id}">
		...
		filtered.forEach((q, i) => {
			...
			const isGFG =
				(q.lcNumber || "").includes("GFG") ||
				(q.lcNumber || "").includes("SPOJ");
			const tr = document.createElement("tr");
			tr.id = "row_" + q._id;
			tr.dataset.topicId = t._id;
			tr.className =
				`${isSolved ? "solved" : ""}${isRevised ? " revised" : ""}`.trim();
			tr.innerHTML = `
...
function renderAiTopics() {
	...
	visibleTopics.forEach((t) => {
		const key = t.phaseIndex || 0;
		if (!phaseGroups[key])
			phaseGroups[key] = { label: t.phase || "", items: [] };
		phaseGroups[key].items.push(t);
	});

	Object.entries(phaseGroups)
		.sort((a, b) => +a[0] - +b[0])
		.forEach(([idx, group], gi) => {
      ...
      `
        ...
        <div class="ai-card-body${isOpen ? " open" : ""}" id="tbody-${t._id}">
          ...
          ${multiQ}
				...
				cardEl.querySelector(".ai-card-header").id =
					"aicard-group-" + t._id;
...
function openModal(topicId, questionId) {
	...
	const isRevised = isQuestionRevised(questionId);
	const isGFG =
		(q.lcNumber || "").includes("GFG") || (q.lcNumber || "").includes("SPOJ");
    ...
    <div class="modal-video-links">${videoLinks
			.map(
				(video) => `
...
function showPopup(msg, icon = "ℹ️") {
	...
	document.getElementById("popupActions").innerHTML =
		`<button class="popup-btn popup-btn-ok" onclick="closePopup()">Got it</button>`;
