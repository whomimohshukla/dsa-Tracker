...
router.get('/', async (req, res) => {
  try {
    ...
    res.json({ success: true, count: progress.length, progress, progressMap });
  } catch (err) {
  ...
  try {
    ...
    res.json({
      ...
      stats: {
        totalSolved: total,
        totalQuestions,
        completionPct: totalQuestions ? Math.round(total / totalQuestions * 100) : 0,
        byDifficulty,
        byTopic,
        recentActivity,
...
router.post('/toggle', async (req, res) => {
  try {
    const { questionId, status = 'solved', notes = '', rating = null } = req.body;
    if (!questionId)
...
async function updateStreak(user) {
  ...
  const lastDate = user.streak.lastSolvedDate
    ? new Date(user.streak.lastSolvedDate.getFullYear(), user.streak.lastSolvedDate.getMonth(), user.streak.lastSolvedDate.getDate())
    : null;
  ...
  if (!lastDate) {
    ...
    else if (diff === 0) return;         // same day, no change
